        ...
        tbody.innerHTML = rows.map(function(d) {
            return '<tr>' +
                ...
                '<td>' + depotOverallBadge(d.status) +
                    (d.updated_at ? '<div data-depot-ts="' + esc(d.patch_id) + '" style="font-size:10px;color:var(--text-dim);margin-top:2px">' + esc(new Date((d.updated_at||'').replace(' ','T')+'Z').toLocaleString('en-ZA', {month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',second:'2-digit'})) + '</div>' : '') +
                '</td>' +
                '<td>' +
    ...
    _depotPollTimers[patch_id] = setInterval(async function() {
        try {
            ...
            if (tbody) {
                ...
                if (tsEl && d.updated_at) tsEl.textContent = new Date((d.updated_at||'').replace(' ','T')+'Z').toLocaleString('en-ZA', {month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',second:'2-digit'});
            }
