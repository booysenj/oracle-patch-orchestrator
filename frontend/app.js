...
document.getElementById('opSelect').addEventListener('change', function() {
  ...
  // Warn only if the agent hasn't discovered any Oracle home yet — staging path cannot be derived
  ...
  if (STAGE_OPS.indexOf(op) >= 0 && selectedVm) {
    var hasHome = selectedVm.old_gi_home || selectedVm.old_db_home || selectedVm.current_db_home;
    var hasStage = selectedVm.stage_path || selectedVm.preferred_staging_mount;
    if (!hasHome && !hasStage) {
      ...
      warn.innerHTML = '&#9888; No Oracle home discovered yet — staging path cannot be derived. ' +
        'Wait for the agent discovery cycle to complete before running this operation.';
      var rcEl = document.getElementById('resolvedConfigPanel');
...
function appendReportRow(label, status, details) {
  ...
  tr.innerHTML =
    ...
    '<td style="padding:7px 12px;font-size:12px;color:var(--text-dim)">' + details + '</td>';
  ...
  await new Promise(resolve => {
    ...
    deployModal.innerHTML =
      '<div style="background:var(--color-bg-primary);border:1px solid var(--color-border-primary);border-radius:10px;width:640px;max-width:95vw;max-height:80vh;display:flex;flex-direction:column">' +
        ...
        '</div>' +
        '<div id="deployLogStatus" style="padding:10px 16px;border-top:1px solid var(--color-border-primary);font-size:12px;color:var(--color-text-secondary)">Running...</div>' +
      '</div>';
    document.body.appendChild(deployModal);
    var logOut = deployModal.querySelector('#deployLogOut');
    var statusEl = deployModal.querySelector('#deployLogStatus');
    try {
      var result = await api('/admin/vms/' + vmId + '/deploy-agent', {
        method: 'POST',
        body: JSON.stringify({ sshUser: user, sshPassword: pass, useSudo })
      });
      logOut.textContent = result.output || '(no output)';
