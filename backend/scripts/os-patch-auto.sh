...
send_db_open_notification() {
    ...
    add_html_row "Database" "INFO" "$db"
    add_html_row "Role" "INFO" "$role"
    add_html_row "Open mode" "INFO" "$mode"
    add_html_row "Oracle home" "INFO" "$home"
    ...
    if [[ "$role" == "PRIMARY" ]]; then
        ...
        if [[ -n "$pdbs" ]]; then
            ...
            add_html_row "Open PDBs" "INFO" "$pdb_html"
        else
            add_html_row "Open PDBs" "INFO" \
