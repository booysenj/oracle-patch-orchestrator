...
router.post('/', (req, res) => {
    const { hostname, ip, ssh_user, ssh_port, node_role, environment, script_path, patch_target } = req.body;
    if (!hostname || !ip) return res.status(400).json({ error: 'hostname and ip required' });
    const id = uuidv4();
    getDB().prepare(
        `INSERT INTO vms (id, hostname, ip, ssh_user, ssh_port, node_role, environment, script_path, patch_target)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(id, hostname, ip, ssh_user || 'root', ssh_port || 22,
          node_role || 'UNKNOWN', environment || 'UAT',
          script_path || '/home/oracle/os-patching-auto-1.sh', patch_target || '19.26');
...
router.put('/:id', (req, res) => {
    ...
    const { hostname, ip, ssh_user, ssh_port, node_role, environment, script_path, patch_target, enabled } = req.body;
    db.prepare(
        `UPDATE vms SET hostname=?, ip=?, ssh_user=?, ssh_port=?, node_role=?,
         environment=?, script_path=?, enabled=?, updated_at=datetime('now') WHERE id=?`
    ).run(
        hostname || existing.hostname, ip || existing.ip,
        ssh_user || existing.ssh_user, ssh_port || existing.ssh_port,
        node_role || existing.node_role, environment || existing.environment,
...
router.post('/bulk', (req, res) => {
    ...
    const insertMany = db.transaction((list) => {
        for (const vm of list) {
            ...
            const id = uuidv4();
            stmt.run(id, vm.hostname, vm.ip, vm.ssh_user || 'root', vm.ssh_port || 22,
